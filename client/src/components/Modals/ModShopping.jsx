...
import { localhost, Sergiogit, Ultimate } from "../../Deploys";
...
const ModShopping = () => {
    ...
    }, [dispatch, idClient, productStatusPending]);   // dispatch, idClient, productStatusPending
            ...
            <Modal isOpen={isOpenShop} closeModal={closeModalShop} isRight={true} >
                        ...
                        <div className="itemsCart">
                            ...
                            <div className="btnconteiner">
                                <button onClick={
                                    (event) => {
                                        event.preventDefault()
                                        axios.post(`${localhost}/payment`, { Title: p.Firstname, Quantity: p.hours, Price: p.Price })
                                            .then((res) =>
                            ...
                            <button className="btnDelete" onClick={
                                (event) => {
                                    event.preventDefault()
                                    axios.put(`${localhost}/User/${idClient}/MyCart/${p.uId}`, { status: "deleted" })
                ...
                {productStatusPending ?
                    <button className="btnPayAll" onClick={
                        (event) => {
                            event.preventDefault()
                            axios.post(`${localhost}/payment`, { Title: "CodeAdvisor", Quantity: 1, Price: sumaPriceTotal })
                                .then((res) =>
                                    (window.location.href = res.data.response.body.init_point));
                        }
                    }
